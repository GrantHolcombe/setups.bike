  ...
  return (
    <div className='suspensionCompression'>
        ...
        <span>
            ...
            <input type='number' className='componentInput' name={detail+`${i+1}`} value={getDetailVal(detail+`${i+1}`, i + 1)} onBlur={(e) =>{if(saveBike) saveBike(e);} } onChange={(event) => valUpdate(event, (i + 1)) }/>
        </span>)})}
    </div>
  );
}